...
def find_minmax_phi_from_pixelvecs (v, basephi):
	pixelthetas, pixelphis = vec2ang(testvecs)
...
class tangent_plane_information:
	...
	def get_pixelvecs(self):
		...
		if self.maxpixelphi == None:
			self.minpixelphi, self.maxpixelphi = find_minmax_phi_from_pixelvecs (testvecs, self.base_phi)
			self.minpixeltheta, self.maxpixeltheta = find_minmax_theta_from_pixelvecs (testvecs)
	...
	def vec_2_tpp_coords (v):
		chob = np.linalg.inv(np.hstack((v1[np.newaxis].T, v2[np.newaxis].T, basevec[np.newaxis].T)))#change of basis matrix.
		secalpha = 1/(np.dot(v.T,basevec))
		tpp_coords_unscaled = secalpha*(chob[:2] @ tempvec)
		return tpp_coords_unscaled[1]/np.tan(extent1), tpp_coords_unscaled[0]/np.tan(extent2) #x,y
...
def tangent_plane_plot (values, tpi, cmap="Greys", vmax=None, logcolor=False, title=None, gridlines=True, plot_chord=False, chord_theta=None, chord_phi=None, plot_source=False, source_phi = None, source_theta = None, colorbar=False):
	...
	if not gridlines: 
		...
		phi_separation = deg_separations[np.searchsorted(deg_thresholds,np.rad2deg(self.maxpixelphi-self.minpixelphi))]
		theta_separation = deg_separations[np.searchsorted(deg_thresholds,np.rad2deg(self.maxpixeltheta-self.minpixeltheta))]
	...
	if plot_source == True:
		x,y = ang_2_tpp_coords (source_theta, source_phi)
